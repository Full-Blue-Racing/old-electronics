...
const byte address[6] = "fullb"; // This is the radio coding that the reciever looks for, dont change
...
int tc_retard;
int launch_timing;
unsigned int temp;                   // This is a temporary variable used to convert the two 8 bit parts of each value to one 16 bit one, not sure why it is a char but I just used the example code from the tutorial 
...
void setup()
{
  ...
  radio.setPALevel(RF24_PA_MAX);  // Power level of radio transmitter, if not properly shielded this will not work and you need to reduce it (change to MIN)
  radio.stopListening();  // This sets it as a transmitter
...
void loop()
{
    if(CAN_MSGAVAIL == CAN.checkReceive())    // Check if data is coming - if having problems look at previous versions for printing statements
    {
        ...
        {
          Serial.println("Can ID does not match any of the datasheet ones"); // Comment this out when done debugging
        }
    // After reading some of the CAN data, we now send all of this via Serial USB to plot on a connected laptop

  char text[140];
  snprintf(text, 140, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", manifold_air_pressure, rpm, clt, tps, pw1, pw2, mat, adv_deg, afrtgt1, AFR1, EGOcor1, egt1, pwseq1, batt, sensors1, sensors2, knk_rtd, VSS1, tc_retard, launch_timing);
  radio.write(&text[0], sizeof(text));  // Dont modify this, not sure how it worked but fudged it
    }
}
